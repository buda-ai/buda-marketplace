...
| Not found | ⛔ **STOP. Do NOT generate any images.** Read [references/config/first-time-setup.md](references/config/first-time-setup.md) and follow its Flow 1 checklist step by step. This is a multi-turn interactive setup that requires asking the user multiple questions. Resume image generation only after Step 5 (verify) passes. |
...
Based on [baoyu-image-gen](https://github.com/JimLiu/baoyu-skills#baoyu-image-gen) by JimLiu, licensed under MIT.
Modified and adapted for the [Buda.im](https://buda.im) platform.

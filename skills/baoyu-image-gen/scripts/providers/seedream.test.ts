...
test("Seedream request body and default extensions follow official model capabilities", () => {
  const five = buildRequestBody("A robot illustrator", "doubao-seedream-5-0-260128", makeArgs());
...
test("Seedream reference-image support is model-specific", () => {
  assert.doesNotThrow(() =>
    validateArgs("doubao-seedream-5-0-260128", makeArgs({ referenceImages: ["a.png", "b.png"] })),
  );

  assert.throws(
    () => validateArgs("doubao-seedream-3-0-t2i-250415", makeArgs({ referenceImages: ["a.png"] })),
    /does not support reference images/,
  );

  assert.throws(
    () => validateArgs("doubao-seededit-3-0-i2i-250628", makeArgs()),
    /no longer supported/,
  );

  assert.throws(
    () => validateArgs("ep-20260315171508-t8br2", makeArgs({ referenceImages: ["a.png"] })),
...
test("Seedream generateImage posts the documented response_format and downloads the returned URL", async (t) => {
  ...
  assert.equal(calls[0]?.input, "https://ark.cn-beijing.volces.com/api/v3/images/generations");

...
import { buildInput, extractOutputUrl, parseModelId } from "./replicate.ts";
...
test("Replicate model parsing accepts official formats and rejects malformed ones", () => {
  ...
  assert.throws(() => parseModelId("just-a-model-name"), /Invalid Replicate model format/);
});
...
test("Replicate input builder maps aspect ratio, image count, quality, and refs", () => {
  ...
  assert.deepEqual(buildInput("A robot painter", makeArgs({ quality: "normal" }), ["ref"]), {
    prompt: "A robot painter",

  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        config.batch ??= {};
        config.batch.provider_limits ??= {};
        config.batch.provider_limits[currentProvider] ??= {};
...
function getModelForProvider(
  ...
  if (extendConfig.default_model) {
    if (provider === "google" && extendConfig.default_model.google)
      return extendConfig.default_model.google;
    if (provider === "openai" && extendConfig.default_model.openai)
      return extendConfig.default_model.openai;
    if (provider === "openrouter" && extendConfig.default_model.openrouter) {
      return extendConfig.default_model.openrouter;
    }
    if (provider === "dashscope" && extendConfig.default_model.dashscope)
      return extendConfig.default_model.dashscope;
    if (provider === "replicate" && extendConfig.default_model.replicate)
      return extendConfig.default_model.replicate;
    if (provider === "jimeng" && extendConfig.default_model.jimeng)
      return extendConfig.default_model.jimeng;
    if (provider === "seedream" && extendConfig.default_model.seedream)
      return extendConfig.default_model.seedream;
  }
...
export async function loadBatchTasks(batchFilePath: string): Promise<LoadedBatchTasks> {
  ...
  throw new Error(
    "Invalid batch file. Expected an array of tasks or an object with a tasks array.",
  );
}
...
export function createTaskArgs(baseArgs: CliArgs, task: BatchTaskInput, batchDir: string): CliArgs {
  return {
    ...
    promptFiles: task.promptFiles
      ? task.promptFiles.map((filePath) => resolveBatchPath(batchDir, filePath))
      : [],
    ...
    referenceImages: task.ref
      ? task.ref.map((filePath) => resolveBatchPath(batchDir, filePath))
      : [],
    n: task.n ?? baseArgs.n,
    batchFile: null,
    jobs: baseArgs.jobs,
...
async function prepareBatchTasks(
  ...
  for (let i = 0; i < taskInputs.length; i++) {
    ...
    const defaultOutputExtension =
      providerModule.getDefaultOutputExtension?.(model, taskArgs) ?? ".png";
    prepared.push({
...
async function runBatchTasks(
  ...
  console.error(
    `Batch mode: ${tasks.length} tasks, ${workerCount} workers, parallel mode enabled.`,
  );
  for (const provider of [
    "replicate",
    "google",
    ...
    const limit = providerRateLimits[provider];
    console.error(
      `- ${provider}: concurrency=${limit.concurrency}, startIntervalMs=${limit.startIntervalMs}`,
    );
  }
